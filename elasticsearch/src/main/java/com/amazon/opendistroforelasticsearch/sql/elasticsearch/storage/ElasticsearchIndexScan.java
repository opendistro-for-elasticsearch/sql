...
public class ElasticsearchIndexScan extends TableScanOperator {
  ...
  /**
   * Util method to decide on the size to scan an index.
   * If size is not specified in query, or the requested size from query is smaller than
   * the query size limit in plugin setting (=200 by default),
   * the scan size would depend on the plugin setting.
