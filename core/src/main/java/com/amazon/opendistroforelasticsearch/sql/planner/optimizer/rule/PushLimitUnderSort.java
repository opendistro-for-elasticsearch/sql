...
@Getter
public class PushLimitUnderSort implements Rule<LogicalLimit> {
  ...
  @Override
  public LogicalPlan apply(LogicalLimit limit,
                           Captures captures) {
    LogicalSort sort = captures.get(capture);
    return new LogicalSort(
        limit.replaceChildPlans(sort.getChild()),
        sort.getSortList()
    );
  }
}
