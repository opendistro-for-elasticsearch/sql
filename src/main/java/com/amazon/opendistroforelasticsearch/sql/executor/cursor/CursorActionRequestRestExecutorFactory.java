...
public class CursorActionRequestRestExecutorFactory {
    ...
    public static CursorRestExecutor createExecutor(RestRequest request, String cursorId, Format format) {
        if (isCloseCursorRequest(request)) {
            return new CursorAsyncRestExecutor(new CursorCloseExecutor(cursorId, format));
        } else {
