...
import com.amazon.opendistro.sql.executor.csv.CSVResult;
import com.amazon.opendistro.sql.executor.csv.CSVResultsExtractor;
...
public class CSVResultsExtractorTests {
    ...
    public void joinSearchResultNotNestedNotFlatNoAggs() throws Exception {
        String query = String.format("select c.gender , h.hname,h.words from %s/gotCharacters c " +
                ...
                "on h.hname = c.house ", TestsConstants.TEST_INDEX_GAME_OF_THRONES, TestsConstants.TEST_INDEX_GAME_OF_THRONES);
        CSVResult csvResult = getCsvResult(false, query);
    ...
    public void dateHistogramTest() throws Exception {
        String query = String.format("select count(*) from %s/online" +
                " group by date_histogram('field'='insert_time','interval'='4d','alias'='days')", TestsConstants.TEST_INDEX_ONLINE);
        CSVResult csvResult = getCsvResult(false, query);
        List<String> headers = csvResult.getHeaders();
        Assert.assertEquals(2, headers.size());
