...
public abstract class PPLIntegTestCase extends SQLIntegTestCase {
  ...
  protected String executeCsvQuery(String query, boolean escape) throws IOException {
    Request request = buildRequest(query,
        QUERY_API_ENDPOINT + String.format(Locale.ROOT, "?format=csv&escape=%b", escape));
  ...
  protected String executeCsvQuery(String query) throws IOException {
    return executeCsvQuery(query, false);
  }
