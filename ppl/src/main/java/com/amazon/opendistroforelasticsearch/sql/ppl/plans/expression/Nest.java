...
public class Nest extends Expression {
    private final Expression currentLayer;
    private final Expression deeperLayer;

    @Override
    public List<Expression> getChild() {
        return Arrays.asList(currentLayer, deeperLayer);
    }

    @Override
    public <R> R accept(NodeVisitor<R> nodeVisitor) {
        if (nodeVisitor instanceof AbstractNodeVisitor) {
            return ((AbstractNodeVisitor<R>) nodeVisitor).visitNest(this);
        } else {
